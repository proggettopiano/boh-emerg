...
### 6. `frontend/src/pages/PdfViewer.jsx` - TIER 2 LOGGING (Regex highlight)
...
### 7. `frontend/src/pages/PdfViewer.jsx` - TIER 2 LOGGING (Favorite toggle)
...
### 8. `frontend/src/pages/Home.jsx` - TIER 2 LOGGING (Search highlight)
**Purpose**: Add console.error to silent catch block (search highlight)  
**Risk**: NONE (just logging, doesn't change logic)
...
|------|------|------|---------|
| `.env.test` | NEW | NONE | Local convenience only (fallback source) |
| `conftest.py` | Update | NONE | Load env vars with safe fallbacks |
| `test_admin_iter3.py` | Update | NONE | Use env var with fallback |
| `test_google_backup.py` | Update | NONE | Use env var with fallback |
| `server.py` | Update | NONE | Warning logging only (NO startup blocking) |
| `PdfViewer.jsx` | Update | NONE | console.error for regex catch |
| `PdfViewer.jsx` | Update | NONE | console.error for favorite catch |
| `Home.jsx` | Update | NONE | console.error for regex catch |

---

## ✅ GUARANTEES (REVISED)

- ✅ **No startup blocking** - All env vars have safe fallbacks
- ✅ **No behavioral differences** - test/prod code paths identical
- ✅ **No runtime changes** - Logging only, same endpoint behavior
- ✅ **Backward compatible** - Works with or without `.env.test`
- ✅ **Single source of truth** - Production env vars on Render/Vercel
- ✅ **Safe for Render cold starts** - No sys.exit() calls
...
```
Production:
  Render/Vercel env vars → backend/server.py (with safe fallbacks)

Local Development:
  .env file (local, not committed) → backend/server.py
  OR .env.test (convenience) → conftest.py

Testing:
  Environment variables → conftest.py (with safe fallbacks)
  Optional: .env.test for convenience
```
...
1. **`.env.test` is convenience only**, not required
   - Tests pass with or without `.env.test`
   - Environment variables are the source of truth
   
2. **Server.py has no startup failures**
   - Warning logged if `ADMIN_LOG_PASSWORD` not set
   - Application continues normally (safe fallback: "Rome02009")
   
3. **All changes are non-breaking**
   - No new dependencies
   - Same behavior in all code paths
   - Logging only additions

4. **Git security**
   - `.env.test` should be added to `.gitignore` (confirm after creation)
   - Production secrets only on Render/Vercel dashboard

---

## 🚀 READY TO APPLY (REVISED)?

All corrections made per your feedback:

✅ No startup failures on missing env vars
✅ Safe fallbacks everywhere
✅ `.env.test` as convenience only
✅ Logging warnings instead of failures
✅ No behavioral differences between test/prod

Ready to proceed with:
1. Create `.env.test`
2. Update test files (conftest, test_admin_iter3, test_google_backup)
3. Update server.py (warning logging only)
4. Update PdfViewer.jsx + Home.jsx (logging only)
5. Run full test suite + build
6. Report results with detailed output
