...
export default function Settings() {
  ...
  const toggleBackup = async () => {
    try { const r = await api.post("/settings/backup", { enabled: !user.backup_enabled }); setUser(r.data); toast.success(`Backup ${r.data.backup_enabled ? "attivato" : "disattivato"}`); }
    catch (e) { toast.error(e.response?.data?.detail || "Errore"); }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 md:px-12 py-12" data-testid="settings-page">
      ...
      <Section title="BACKUP" testId="settings-backup-section">
        <div className="flex items-center justify-between gap-4 p-4 border border-rule rounded-sm">
          <div>
            <p className="font-medium">Backup automatico</p>
            <p className="text-sm text-muted2 max-w-md">Quando OFF i file restano solo sul server. Se viene perso, i PDF caricati saranno persi.</p>
          </div>
          <button onClick={toggleBackup} className={`px-4 py-2 rounded-sm font-medium text-sm ${user.backup_enabled ? "bg-emerald-500 text-white" : "bg-canvas3 text-ink border border-rule"}`} data-testid="settings-backup-toggle">
            {user.backup_enabled ? "ATTIVO" : "OFF"}
          </button>
        </div>
        {!user.backup_enabled && (
          <div className="mt-3 flex items-start gap-2 text-highlightFg bg-highlight border border-[#FDE047] p-3 rounded-sm text-sm">
            ...
            <span>Backup disattivato. Senza backup, in caso di perdita del server tutti i PDF dovranno essere ricaricati manualmente.</span>
          </div>
        )}
      </Section>
